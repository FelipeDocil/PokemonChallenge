...
class PokemonListView: UIViewController, PokemonListViewInput {
    ...
    lazy var refreshControl: UIRefreshControl = {
        let tmpRefresh = UIRefreshControl(frame: .zero)
        tmpRefresh.addTarget(self, action: #selector(didPullToRefresh(sender:)), for: .valueChanged)
        tmpRefresh.tintColor = .black
        tmpRefresh.attributedTitle = NSAttributedString(string: "Fetching pokemon...")
    ...
    lazy var collectionView: UICollectionView = {
        ...
        tmpCollectionView.alwaysBounceVertical = true
        tmpCollectionView.backgroundColor = .white
        tmpCollectionView.keyboardDismissMode = .onDrag
        tmpCollectionView.refreshControl = refreshControl
    ...
    @objc private func didPullToRefresh(sender: UIRefreshControl) {
        print("💸 That's a £\(#line) worth of code you're messin' in at \(#file) \n" +
                "sender \(sender) \n" +
                "💸"
        )
        presenter.loadNewPokemon()
    }
}
